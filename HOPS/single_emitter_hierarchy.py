...
class SingleParticleHierarchy(HOPSHierarchy):
    ...
    def solve_linear_HOPS(self, start: float, end: float, initial_state: np.ndarray, H: Callable[[float, np.ndarray], np.ndarray], z: Callable[[float], complex], custom_operators: Callable[[float, np.ndarray], np.ndarray] = [], diffusion_operators: list[Callable[[float, np.ndarray], np.ndarray]] = [], white_noises: list[Callable[[float], complex]] = [], step_size: float = 0.01) -> tuple[np.ndarray, np.ndarray]:
        calculation = _HOPSCalculationSingleEmitter(self, H, z, None, custom_operators, diffusion_operators)
    ...
    def solve_non_linear_HOPS(self, start: float, end: float, initial_state: np.ndarray, H: Callable[[float, np.ndarray], np.ndarray], z: Callable[[float], complex], shift_type: str | None = None, custom_operators: Callable[[float, np.ndarray], np.ndarray] = [], diffusion_operators: list[Callable[[float, np.ndarray], np.ndarray]] = [], white_noises: list[Callable[[float], complex]] = [], step_size: float = 0.01) -> tuple[np.ndarray, np.ndarray]:
        ...
        calculation = _HOPSCalculationSingleEmitter(self, H, z, shift_type, custom_operators, diffusion_operators, noise_shifted=True)
